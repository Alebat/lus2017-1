...
cat corpus2.csv | sort | uniq -c | sort -k 2 | sed -r s/" +"/""/ | sed s/" "/"\t"/ | tail -n +2 | sort -nr > lexi-counts.csv
cat corpus2.csv | cut -f 2 | sort | uniq | sed '/^$/d' | sed 's/.*/1\t<unk>\t&/' >> lexi-counts.csv
...
# Evaluation
cat prediction.csv | tr '\t' ' ' | ./conlleval.pl -l > eval.fast.txt
