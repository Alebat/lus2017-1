...
cat NLSPARQL.train.data | sort | uniq -c | sort -k 2 | sed -r s/" +"/""/ | sed s/" "/"\t"/ | tail -n +2 > lexi-counts.pre.csv
cat lexi-counts.pre.csv | sort -nr |sed '/^1\t/d' > lexi-counts.csv
...
# Evaluation
cat prediction.csv | tr '\t' ' ' | ./conlleval.pl > eval.cut1.txt
