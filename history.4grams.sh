...
cat NLSPARQL.train.data | sort | uniq -c | sort -k 2 | sed -r s/" +"/""/ | sed s/" "/"\t"/ | tail -n +2 | sort -nr > lexi-counts.csv
...
ngramcount --order=4 --require_symbols=false tagphrases.far > ngrams.far
...
# Evaluation
cat prediction.csv | tr '\t' ' ' | ./conlleval.pl > eval.4grams.txt
